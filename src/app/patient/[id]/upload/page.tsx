	...
	const handleUploadAndProcess = async () => {
		...
		try {
			...
			if (env.NEXT_PUBLIC_USE_MOCK_DATA) {
				...
				uploadResult = await uploadOnlyMutation.mutateAsync({
					patientId,
					files: base64Files,
				});
			...
			if (uploadResult && Array.isArray(uploadResult)) {
				for (const image of uploadResult) {
					if (image.results) {
						...
						const activityValue = await boostService.calculateMriUpload(
							results
						);
								...
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-xs text-orange-700 mt-1">
											...
											results. This should connect to the real server that
											actually works for MRI analysis, but we were unable to
											deploy it here due to free account limitations
										</p>
									</div>
								</div>
							</CardContent>
					...
					<Card>
						...
						<CardContent className="space-y-4">
							<div className="space-y-2">
								<Label htmlFor="files">
									Select MRI files (DICOM/NIfTI/JPEG/PNG)
								</Label>
								{/* <UploadDropzone
									...
									onClientUploadComplete={(res) => {
										...
										toast.success(
											`Successfully uploaded ${res.length} file(s)`
										);
										// Refetch images to show the new uploads
										imagesQuery.refetch();
									}}
							...
							<div className="flex justify-end">
								<Button
									...
									{isProcessing
										...
										? "Uploading..."
										: `Upload & ${
												env.NEXT_PUBLIC_USE_MOCK_DATA ? "Simulate" : "Process"
										  }`}
								</Button>
							</div>
						</CardContent>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									...
									{env.NEXT_PUBLIC_USE_MOCK_DATA &&
										" (Showing real database data only)"}
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
															...
															<div className="flex items-center justify-between">
																...
																{results?.predicted_label && (
																	<Badge
																		...
																		{results.predicted_label.replaceAll(
																			"_",
																			" "
																		)}
																	</Badge>
