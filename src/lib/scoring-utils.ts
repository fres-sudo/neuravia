import { BoostScoreService, ScoreInsertData } from "@/scoring/service";
 ...
 */
export async function calculateMriScore(probabilities: number[]): Promise<number> {
 ...
 */
export async function calculateWeeklyFormScore(responses: number[]): Promise<number> {
 ...
 */
export function getActivityWeight(activityType: 'initial_assessment' | 'mri_upload' | 'weekly_form' | 'game_played'): number {
 ...
 */
export function calculateNewScore(previousScore: number, activityValue: number, weight: number): number {
