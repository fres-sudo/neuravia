...
export class BoostScoreService {
	...
	async calculateWeeklyForm(responses: any): Promise<number> {
		const avgScore =
			responses.reduce((sum: number, val: number) => sum + val, 0) /
			responses.length;
	...
	async calculateGamePlayed(sessionData: any): Promise<number> {
		...
		const { averageScore, gamesCompleted, rawData } = sessionData;
		...
		const scores = Object.values(rawData)
			.map((gameData: any) => {
				if (Array.isArray(gameData.rounds)) {
					return (
						gameData.rounds.reduce(
							(sum: number, round: any) => sum + round.score,
							0
	...
	private calculateBaseScore(diagnosis: string, stage: string): number {
		...
		let score =
			diagnosisScores[diagnosis as keyof typeof diagnosisScores] || 50;

		// Apply stage modifier only if there's a diagnosis
		if (diagnosis !== "None" && stage) {
			const modifier =
				stageModifiers[stage as keyof typeof stageModifiers] || 0;
			score += modifier;
		}
	...
	private async adjustScoreWithLLM(
		baseScore: number,
		notes: string,
		diagnosis: string,
		stage: string
		...
		console.log(
			"Adjusting score with LLM, baseScore:",
			baseScore,
			"notes:",
			notes
		);
